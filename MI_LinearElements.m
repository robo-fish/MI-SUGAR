...
- (void) draw
{
    ...
    [super draw];
    [bp moveToPoint:NSMakePoint(self.position.x - 21.0f, self.position.y)];
    [bp relativeLineToPoint:NSMakePoint(  8.0f, 0.0f)];
    [bp relativeMoveToPoint:NSMakePoint( 26.0f, 0.0f)];
    [bp relativeLineToPoint:NSMakePoint(  8.0f, 0.0f)];
    [bp stroke];
    [NSBezierPath strokeRect:NSMakeRect(self.position.x - 13.0f, self.position.y - 6.0f, 26.0f, 12.0f)];
    [super endDraw];
}
...
- (NSString*) shapeToSVG
{
    ...
    [svg appendFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g h 8.0 m 26.0 0.0 h 8.0\"/>",
        [super shapeToSVG], self.position.x - 21.0f, self.position.y];
    [svg appendFormat:@"\n<rect x=\"%g\" y=\"%g\" width=\"26.0\" height=\"12.0\"/>%@",
        self.position.x - 13.0f, self.position.y - 6.0f, [super endSVG]];
    return svg;
}
...
- (NSString*) shapeToSVG
{
    return [NSString stringWithFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g h 8.0 l 2.17 6.0 l 4.33 -12.0 l 4.33 12.0 l 4.33 -12.0 l 4.33 12.0 l 4.33 -12.0 l 2.17 6.0 h 8.0\"/>%@",
        [super shapeToSVG], self.position.x - 21.0f, self.position.y, [super endSVG]];
}
...
- (void) draw
{
    ...
    [bp moveToPoint:NSMakePoint(self.position.x + 13.0f, self.position.y + 14.0f)];
...
- (NSString*) shapeToSVG
{
  ...
  [svg appendFormat:@"%@<path d=\"M %g %g h 8.0 m 26.0 0.0 h 8.0\"/>",
      [super shapeToSVG], self.position.x - 21.0f, self.position.y];
  [svg appendFormat:@"\n<rect x=\"%g\" y=\"%g\" width=\"26.0\" height=\"12.0\"/>",
      self.position.x - 13.0f, self.position.y - 6.0f];
  // draw arrow
  [svg appendFormat:@"\n<line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" />",
      self.position.x - 13.0f, self.position.y - 14.0f, self.position.x + 13.0f, self.position.y + 14.0f];
  [svg appendFormat:@"\n<path stroke=\"black\" fill=\"black\" d=\"M %g %g l -2.0 -6.5 l -4.0 4.0\"/>%@",
      self.position.x + 13.0f, self.position.y + 14.0f, [super endSVG]];
  return svg;
}
...
- (void) draw
{
  NSBezierPath* bp = [NSBezierPath bezierPath];
  [super draw];
  [bp moveToPoint:NSMakePoint(self.position.x - 21.0f, self.position.y - 6.0f)];
  [bp relativeMoveToPoint:NSMakePoint(  0.0f,  6.0f)];
  [bp relativeLineToPoint:NSMakePoint(  8.0f,  0.0f)];
  [bp relativeMoveToPoint:NSMakePoint( 26.0f,  0.0f)];
  [bp relativeLineToPoint:NSMakePoint(  8.0f,  0.0f)];
  [bp stroke];
  [NSBezierPath fillRect:NSMakeRect(self.position.x - 13.0f, self.position.y - 6.0f, 26.0f, 12.0f)];
  [super endDraw];
}
...
- (void) draw
{
    ...
    [bp appendBezierPathWithArcWithCenter:NSMakePoint(self.position.x + 9.75f, self.position.y)
                                   radius:3.25f
                               startAngle:0.0f
                                 endAngle:180.0f];
    [bp appendBezierPathWithArcWithCenter:NSMakePoint(self.position.x + 3.25f, self.position.y)
                                   radius:3.25f
                               startAngle:0.0f
                                 endAngle:180.0f];
    [bp appendBezierPathWithArcWithCenter:NSMakePoint(self.position.x - 3.25f, self.position.y)
...
- (NSString*) shapeToSVG
{
    return [NSString stringWithFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g h 8 v 6 m 6.5,0 v -6 m 6.5,0 v 6 m 6.5,0 v -6 m 6.5,6 v -6 h 8 m -8,6 a 3.25 3.25 0 0 1 -6.5,0 a 3.25 3.25 0 0 1 -6.5,0 a 3.25 3.25 0 0 1 -6.5,0 a 3.25 3.25 0 0 1 -6.5,0\"/>%@",
        [super shapeToSVG], self.position.x - 21.0f, self.position.y - 6.0f, [super endSVG]];
}
...
- (void) draw
{
    ...
    // Draw left coil
    [bp moveToPoint:NSMakePoint(self.position.x - 18.0f, self.position.y + 21.0f)];
    ...
    [bp appendBezierPathWithArcWithCenter:NSMakePoint(self.position.x - 12.0f, self.position.y - 9.75f)
                                   radius:3.25f
                               startAngle:-90.0f
                                 endAngle:90.0f];
    [bp appendBezierPathWithArcWithCenter:NSMakePoint(self.position.x - 12.0f, self.position.y - 3.25f)
                                   radius:3.25f
                               startAngle:-90.0f
                                 endAngle:90.0f];
    [bp appendBezierPathWithArcWithCenter:NSMakePoint(self.position.x - 12.0f, self.position.y + 3.25f)
    ...
    // Draw coupling lines
    [bp moveToPoint:NSMakePoint(self.position.x + 2.0f, self.position.y + 13.0f)];
    ...
    [[NSBezierPath bezierPathWithOvalInRect:NSMakeRect(self.position.x - 15.0f, self.position.y + 18.0f, 3.0f, 3.0f)] fill];
...
- (NSString*) shapeToSVG
{
  ...
  [svg appendFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g v -8 h 6 m 0 -6.5 h -6 m 0 -6.5 h 6 m 0 -6.5 h -6 m 6 -6.5 h -6 v -8 m 6 8 a 3.25 3.25 0 0 1 0 6.5 a 3.25 3.25 0 0 1 0 6.5 a 3.25 3.25 0 0 1 0 6.5 a 3.25 3.25 0 0 1 0 6.5\"/>",
      [super shapeToSVG], self.position.x - 18.0f, self.position.y + 21.0f];
  // right coil
  [svg appendFormat:@"\n<path stroke=\"black\" fill=\"none\" d=\"M %g %g v -8 h -6 m 0 -6.5 h 6 m 0 -6.5 h -6 m 0 -6.5 h 6 m -6 -6.5 h 6 v -8 m -6 8 a 3.25 3.25 0 0 0 0 6.5 a 3.25 3.25 0 0 0 0 6.5 a 3.25 3.25 0 0 0 0 6.5 a 3.25 3.25 0 0 0 0 6.5\"/>",
      self.position.x + 18.0f, self.position.y + 21.0f];
  // coupling lines
  [svg appendFormat:@"\n<path stroke=\"black\" fill=\"none\" d=\"M %g %g v -26 m -4 0 v 26\"/>",
      self.position.x + 2.0f, self.position.y + 13.0f];
  // primary side indicator
  [svg appendFormat:@"\n<circle stroke=\"black\" fill=\"black\" cx=\"%g\" cy=\"%g\" r=\"1.5\"/>%@",
      self.position.x - 13.5f, self.position.y + 19.5f, [super endSVG]];
  return svg;
}
...
- (void) draw
{
  ...
  // Draw left coil
  [bp moveToPoint:NSMakePoint(self.position.x - 12.0f, self.position.y + 21.0f)];
  [bp relativeLineToPoint:NSMakePoint( 0.0f, -8.0f)];
  [bp relativeMoveToPoint:NSMakePoint( 0.0f,-26.0f)];
  [bp relativeLineToPoint:NSMakePoint( 0.0f, -8.0f)];
  [NSBezierPath fillRect:NSMakeRect(self.position.x - 18.0f, self.position.y - 13.0f, 12.0f, 26.0f)];
  ...
  [bp relativeMoveToPoint:NSMakePoint( 0.0f,-26.0f)];
  [bp relativeLineToPoint:NSMakePoint( 0.0f, -8.0f)];
  [NSBezierPath fillRect:NSMakeRect(self.position.x + 6.0f, self.position.y - 13.0f, 12.0f, 26.0f)];
  ...
  // Draw primary side indicator
  [[NSBezierPath bezierPathWithOvalInRect:NSMakeRect(self.position.x - 9.0f, self.position.y + 18.0f, 3.0f, 3.0f)] fill];
...
- (NSString*) shapeToSVG
{
  ...
  [svg appendFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g v -8 m 0 -26 v -8\"/>",
      [super shapeToSVG], self.position.x - 12.0f, self.position.y + 21.0f];
  [svg appendFormat:@"\n<rect stroke=\"black\" fill=\"black\" x=\"%g\" y=\"%g\" width=\"12\" height=\"26\"/>",
      self.position.x - 18.0f, self.position.y - 13.0f];
  // right coil
  [svg appendFormat:@"\n<path stroke=\"black\" fill=\"none\" d=\"M %g %g v -8 m 0 -26 v -8\"/>",
      self.position.x + 12.0f, self.position.y + 21.0f];
  [svg appendFormat:@"\n<rect stroke=\"black\" fill=\"black\" x=\"%g\" y=\"%g\" width=\"12\" height=\"26\"/>",
      self.position.x + 6.0f, self.position.y - 13.0f];
  // coupling lines
  [svg appendFormat:@"\n<path stroke=\"black\" fill=\"none\" d=\"M %g %g v -26 m -4 0 v 26\"/>",
      self.position.x + 2.0f, self.position.y + 13.0f];
  // primary side indicator
  [svg appendFormat:@"\n<circle stroke=\"black\" fill=\"black\" cx=\"%g\" cy=\"%g\" r=\"1.5\"/>%@",
      self.position.x - 7.5f, self.position.y + 19.5f, [super endSVG]];
  return svg;
}
...
- (NSString*) shapeToSVG
{
  ...
  [svg appendFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g h 13 m 6 0 h 13 m -19 -12 v 24 m 6 0 v -24\"/>%@",
      [super shapeToSVG], self.position.x - 16.0f, self.position.y, [super endSVG]];
  return svg;
}
...
- (void) draw
{
  ...
  [bp moveToPoint:NSMakePoint(self.position.x - 25.0f, self.position.y + 9.0f)];
  [bp relativeLineToPoint:NSMakePoint(  8.0f,   0.0f)];
  ...
  [bp moveToPoint:NSMakePoint(self.position.x, self.position.y + 12.0f)];
  ...
  [bp stroke];
  // Draw frame
  [NSBezierPath strokeRect:NSMakeRect(self.position.x - 17.0f, self.position.y - 12.0f, 34.0f, 24.0f)];
  // Draw primary side indicator
  [[NSBezierPath bezierPathWithOvalInRect:NSMakeRect(self.position.x - 14.0f, self.position.y + 7.0f, 3.0f, 3.0f)] fill];
  [super endDraw];
}
...
- (NSString*) shapeToSVG
{
  ...
  [svg appendFormat:@"%@<path stroke=\"black\" fill=\"none\" d=\"M %g %g h 8 m 0 -18 h -8 m 42 0 h 8 m 0 18 h -8\"/>",
      [super shapeToSVG], self.position.x - 25.0f, self.position.y + 9.0f];
  // middle vertical line
  [svg appendFormat:@"<path stroke-width=\"2\" stroke=\"black\" fill=\"none\" d=\"M %g %g v -5 m 0 -4.5 v -5 m 0 -4.5 v -5\"/>",
      self.position.x, self.position.y + 12.0f];
  // frame
  [svg appendFormat:@"\n<rect stroke=\"black\" fill=\"none\" x=\"%g\" y=\"%g\" width=\"34\" height=\"24\"/>",
      self.position.x - 17.0f, self.position.y - 12.0f];
  // primary side indicator
  [svg appendFormat:@"\n<circle stroke=\"black\" fill=\"black\" cx=\"%g\" cy=\"%g\" r=\"1.5\"/>%@",
      self.position.x - 12.5f, self.position.y + 8.5f, [super endSVG]];
  return svg;
}
