...
+ (MI_CircuitDocumentPrinter*) sharedPrinter
{
  ...
  {
    circuitPrinter = [[MI_CircuitDocumentPrinter alloc] init];
  }
...
- (void) runPrintSheetForCircuit:(CircuitDocument*)circuit
{
  ...
  [currentInfo setOrientation:NSPaperOrientationPortrait];
...
//    screenSizeInPixels.height, screenSizeInPoints.width, screenSizeInPoints.height);
  ...
  NSWindow* window = [[[circuit windowControllers] objectAtIndex:0] window];
  [window beginSheet:circuitPrintOptionsSheet completionHandler:^(NSModalResponse returnCode) {
    /* do nothing */
  }];
}
...
- (IBAction) commit:(id)sender
{
  ...
  if (sender == commitButton)
  {
    ...
    NSPrintOperation *op = [NSPrintOperation printOperationWithView:currentTargetView printInfo:currentInfo];
...
{
  BOOL const isLandscape = [[[sender selectedCell] title] isEqualToString:@"Landscape"];
  [currentInfo setOrientation: isLandscape ? NSPaperOrientationLandscape : NSPaperOrientationPortrait];
...
- (void) drawRect:(NSRect)rect
{
    ...
    NSRect paperRect;
    CGFloat const widthHeightRatio = [currentInfo paperSize].width / [currentInfo paperSize].height;
    if (currentInfo && ([currentInfo orientation] == NSPaperOrientationLandscape))
    {
      // Landscape
    ...
    const CGFloat dashPattern[] = {3.0f, 3.0f};
    [marginPath setLineDash:dashPattern count:1 phase:0.0f];
