...
- (id) initWithNumberOfPins:(int)numPins
{
    if (self = [super init])
    {
        ...
        if (numberOfPins % 2)
            numberOfPins++;
        size = NSMakeSize(10.0f * numberOfPins / 2, 36.0f);
        ...
        {
            if (i > (numberOfPins / 2))
                pointPosition = NSMakePoint(size.width/2 + 5.0f - (i - numberOfPins/2) * 10.0f,
                                            size.height/2);
            else
                pointPosition = NSMakePoint(i * 10.0f - 5.0f - size.width/2,
                                            -size.height/2);
            ...
            [points addObject:[[[MI_ConnectionPoint alloc]
                initWithPosition:pointPosition
                            ...
                            name:[NSString stringWithFormat:@"Pin%d", i]] autorelease]];
        }
        [self setConnectionPoints:
            [[NSDictionary alloc] initWithObjects:points
                                          forKeys:pointNames]];
    }
    return self;    
}
...
{
    int pinsPerSide = numberOfPins / 2;
    int i;
    NSBezierPath* bp;
    ...
    if (name != nil)
    {
        NSSize nameStringSize = [name sizeWithAttributes:[MI_SchematicElement labelFontAttributes]];
        NSPoint place = NSMakePoint(position.x + (NOTCH_RADIUS - nameStringSize.width)/2.0f,
                                    position.y - nameStringSize.height/2.0f);
        if (nameStringSize.width < size.width - NOTCH_RADIUS)
            /* can not use because of text clipping bug in Cocoa
            [name drawAtPoint:place
               withAttributes:[MI_SchematicElement labelFontAttributes]];
             */
            [MI_TroubleShooter drawString:name
                               attributes:[MI_SchematicElement labelFontAttributes]
                                  atPoint:place
                                 rotation:0.0f];
    }
}
...
- (id)initWithCoder:(NSCoder *)decoder
{
    ...
    {
        name = [[decoder decodeObjectForKey:@"DIPShapeName"] retain];
        numberOfPins = [decoder decodeIntForKey:@"DIPShapePinCount"];
    }
    return self;
}
